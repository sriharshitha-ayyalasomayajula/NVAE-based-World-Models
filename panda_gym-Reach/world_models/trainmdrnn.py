...
# Loading VAE
# vae_file = join(args.logdir, 'nvae', 'best.tar')
# assert exists(vae_file), "No trained VAE in the logdir..."
# state = torch.load(vae_file, map_location=device)
# print("Loading VAE")

# vae = NVAE(LSIZE, (RED_SIZE, RED_SIZE))
# vae.apply(add_sn)
# vae.to(device)
...
mdrnn = MDRNN(LSIZE, ASIZE, RSIZE, 8)
mdrnn.to(device)
...
train_loader = DataLoader(
    RolloutSequenceDataset('dataset/panda_data', SEQ_LEN, transform, buffer_size=30),
    batch_size=BSIZE, num_workers=8, shuffle=True, drop_last=True)
test_loader = DataLoader(
    RolloutSequenceDataset('dataset/panda_data', SEQ_LEN, transform, train=False, buffer_size=10),
...
def to_latent(obs, next_obs):
    ...
    with torch.no_grad():
        ...
        #     vae(x)[3:] for x in (obs, next_obs)]

        obs_mu, obs_logsigma = obs_mu.squeeze(), obs_logsigma.squeeze()
        next_obs_mu, next_obs_logsigma = next_obs_mu.squeeze(), next_obs_logsigma.squeeze()
...
def data_pass(epoch, train, include_reward): # pylint: disable=too-many-locals
    ...
    for i, data in enumerate(loader):
        ...
        # convert BxD to Bx1xD
        # obs = obs.unsqueeze(1)
        #obs = torch.cat((obs, torch.zeros(obs.shape[0], obs.shape[1], 7).to(device)), dim=2)
        #next_obs = torch.cat((next_obs, torch.zeros(next_obs.shape[0], next_obs.shape[1], 7).to(device)), dim=2)

        # transform obs
        latent_obs, latent_next_obs = obs, next_obs
