...
class dataset(torch.utils.data.Dataset):
    def __init__(self, data_path):
        ...
        for i in os.listdir(data_path)[:600]:
            if i.endswith(".npz"):
...
if __name__ == '__main__':
    ...
    train_ds = dataset("/home/pagadalam/Santhosh/Workspace/Fake_Assignments/world-models-nvae/Panda/world-models/dataset/panda_data")
    print(len(train_ds))
    ...
    for epoch in range(epochs):
        ...
        for i, image in enumerate(train_dataloader):
            optimizer.zero_grad()
            # change image shape to BxD to Bx1xD
            image = image.unsqueeze(1)

            image=torch.cat((image,torch.zeros((image.shape[0],1,7))),dim=2)
            # print(image.shape)
            image = image.to(device)
            image_recon, recon_loss, kl_losses, _, _ = model(image)
            image_recon=image_recon[:,:,25]
