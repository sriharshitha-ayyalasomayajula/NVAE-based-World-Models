...
class Decoder(nn.Module):
    ...
    def __init__(self, z_dim):
        ...
        self.condition_z = nn.ModuleList([
            nn.Sequential(
                ...
                Swish(),
                nn.Conv1d(z_dim // 2, z_dim, kernel_size=1)
            ),
            nn.Sequential(
                ResidualBlock(z_dim // 8),
                Swish(),
                nn.Conv1d(z_dim // 8, z_dim // 4, kernel_size=1)
            )
        ])
        ...
        self.condition_xz = nn.ModuleList([
            ...
            nn.Sequential(
                ...
                nn.Conv1d(z_dim // 4, z_dim // 8, kernel_size=1),
                Swish(),
                nn.Conv1d(z_dim // 8, z_dim // 4, kernel_size=1)
            )
        ])
        ...
        self.recon = nn.Sequential(
            ...
            nn.Conv1d(z_dim // 32, 1, kernel_size=1),
        )
    ...
    def forward(self, z, xs=None, mode="random", freeze_level=-1):
        ...
        decoder_out = torch.zeros(B, D, map_h, device=z.device, dtype=z.dtype)
        ...
        for i in range(len(self.decoder_residual_blocks)):
            # print("at i:",i,"decoder_out.shape", decoder_out.shape, "z.shape", z.shape)
            ...
            if xs is not None:
                # print("At i :",i,"xs[i].shape", xs[i].shape, "decoder_out.shape", decoder_out.shape)
